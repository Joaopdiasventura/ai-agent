...
func performAskRequest(t *testing.T, body string) *httptest.ResponseRecorder {
	...
	answer := func(question string) (agent.Result, error) {
		switch question {
		case "Me fale sobre o auronix":
			return agent.Result{
				Response: "Auronix é um projeto de banco digital.",
			}, nil
		case "Unrelated question":
			return agent.Result{}, errors.New("I don't have that specific information")
		default:
			return agent.Result{
	...
	Aswer(answer).ServeHTTP(response, request)
